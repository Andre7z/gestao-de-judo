...
# So' para a aula: cria as tabelas que ainda nao existem ao subir.
# Ele NAO sabe alterar uma tabela que ja' existe:
...
def traduzir_recusa_de_atividade(
    request: Request,
    erro: ErroAtividade
):
    """O unico lugar do sistema que transforma recusa de atividade em HTTP."""

    codigo = (
        404
        if isinstance(erro, AtividadeNaoEncontrada)
        else 409
    )
